...
const userApi = {
  ...
  getStatus: async (id, onSuccess, onError) => {
    try {
      let res = await Axios().get(`/api/user/getStatus/${id}`);
      onSuccess(res.data.data);
    } catch (error) {
      console.log(error.response);
      onError(error.response.data);
  ...
  insertRide: async (id, payload, onSuccess, onError) => {
    try {
      let res = await Axios().post(`/api/user/insert-ride`, {
        ...payload,
      });
      onSuccess(res.data.data);
    } catch (error) {
