...
const MapComp = () => {
  ...
  const changeRideType = (e) => {
    if (!status && e.target.innerText === "Offer Ride") {
      alert("please upload documents first");
    } else {
      if (e.target.innerText === "Find Ride") {
        setRideType("find");
      } else if (status && e.target.innerText === "Offer Ride") {
        setRideType("offer");
      }
  ...
  const gotoBookRide = () => {
    if (rideType === "offer") {
      navigate(
        `/offer-ride?start=${JSON.stringify(
          startLocation
        )}&end=${JSON.stringify(endLocation)}&id=${JSON.stringify(
          authState.id
        )})}`
      );
      return;
    ...
    navigate(
      `/book-ride?start=${JSON.stringify(startLocation)}&end=${JSON.stringify(
        endLocation
      )}&id=${JSON.stringify(authState.id)})}`
    );
  };
  return (
    <>
